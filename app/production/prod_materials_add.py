...
from app.production.prod_model import Production , ProductionBasic , ProductionActivity, ProductionMaterials , ProductionMaterialsSchema
from app.production.prod_model import ProdRawItem, ProdFinishedItem, ProdAccessoriesItem, ProdOtherMaterialsItem \
    , Uom , RawGoods , FinishedGoods , Accessories , OtherMaterials  , ProdAccessoriesItemSchema
...
def get_materials(id):
    ...
    schema = ProductionMaterialsSchema(many=True)
...
def add_materials():
    if request.method == 'POST':
        ...
        basic_data = ProductionBasic.query.filter_by(id = int(basic_id)).first()
        activity_data = ProductionActivity.query.filter_by(id=int(activity_id)).first()
        if payload:
            try:
                new_data = ProductionMaterials()
                final_Data = Production()
                basic_data = ProductionBasic.query.filter_by(id = int(basic_id)).first()
                activity_data = ProductionActivity.query.filter_by(id=int(activity_id)).first()
                basic_data.production = final_Data
                activity_data.production = final_Data
                new_data.production = final_Data
...
def update_materials(id):
    if request.method == 'POST':
        ...
        if payload:
            try:
                new_data = Production.query.filter_by(id=int(id)).first().material[0]
