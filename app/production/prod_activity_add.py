...
from app.production.prod_model import Production, ProductionActivity,ProdTaskItemAct, ProductionActivitySchema
...
def get_activity(id):
    ...
    data = trans.activity
    schema = ProductionActivitySchema(many=True)
...
def update_activity_item(item_id):
    ...
    try:
        item = ProdTaskItemAct.query.filter_by(id=int(item_id)).first()
        item.flag = "done"
        db.session.commit()
        db.session.close()
            ...
            try:
                ...
                for key, item in enumerate(payload['task_items']):
                    ...
                    new_data.task_items_act.append(
                        ProdTaskItemAct(int(key), item['name'], department, location, int(item['days']), int(item['depends']), item['remarks'], start_date, end_date , item['flag']))
...
def update_activity(id):
    if request.method == 'POST':
        ...
        if payload:
            try:
                new_data = Production.query.filter_by(id=int(id)).first().activity[0]
                print(new_data)
