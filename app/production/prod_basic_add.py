...
from app.production.prod_model import Production, ProductionBasic, ProductionBasicSchema
...
def get_basic(id):
    ...
    schema = ProductionBasicSchema(many=True)
        ...
        if payload:
            try:
                ...
                foldertemp = os.path.join(
                    UPLOAD_FOLDER, 'production', str(gen_folder_name))
    ...
    if request.method == 'POST':
        ...
        if payload:
            try:
                ...
                trans = Production.query.filter_by(
                    id=int(id)).first()
