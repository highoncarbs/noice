...
from app.basic_master.model import FabricProcess, FabricProcessSchema
...
@bp.route('/get/fabric_process', methods=['GET'])
@login_required
def get_fabric_process():
    if request.method == 'GET':

        data_schema = FabricProcessSchema(many=True)
        data = FabricProcess.query.all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/fabric_process', methods=['POST'])
@login_required
def add_fabric_process():
    if request.method == 'POST':
        payload = request.json
        print(payload)
        if len(payload['name']) != 0:

            check_data = FabricProcess.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
                return jsonify({'message': 'Product Category - '+check_data.first().name+' already exists.'})
            else:
                try:
                    new_data = FabricProcess(
...
@bp.route('/edit/fabric_process', methods=['POST'])
@login_required
def edit_fabric_process():
    if request.method == 'POST':
        
        payload = request.json
        if payload['name'] is not None:

            check_data = FabricProcess.query.filter_by(
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = FabricProcess.query.filter_by(
                        id=payload['id']).first()
...
@bp.route('/delete/fabric_process', methods=['POST'])
@login_required
def delete_fabric_process():
    if request.method == 'POST':
        payload = request.json
        check_data = FabricProcess.query.filter_by(id=payload['id'])
        if check_data.first():
            if len(check_data.first().fabric_process_goods) is int(0):
