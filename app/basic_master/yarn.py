...
from app.basic_master.model import Yarn, YarnSchema
...
@bp.route('/get/yarn', methods=['GET'])
@login_required
def get_yarn():
    if request.method == 'GET':

        data_schema = YarnSchema(many=True)
        data = Yarn.query.all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/yarn', methods=['POST'])
@login_required
def add_yarn():
    if request.method == 'POST':
        ...
        if len(payload['name']) != 0:
            ...
            check_data = Yarn.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
...
@bp.route('/edit/yarn', methods=['POST'])
@login_required
def edit_yarn():
    if request.method == 'POST':
        ...
        if payload['name'] is not None:
            ...
            check_data = Yarn.query.filter_by(
...
@bp.route('/delete/yarn', methods=['POST'])
@login_required
def delete_yarn():
    if request.method == 'POST':
        payload = request.json
        check_data = Yarn.query.filter_by(id=payload['id'])
        if check_data.first():
            if len(check_data.first().yarn_goods) is int(0):
                try:
                    check_data.delete()
                    db.session.commit()
