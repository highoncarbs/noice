...
from app.basic_master.model import FabricCombination, FabricCombinationSchema
...
@bp.route('/get/fabric_combination', methods=['GET'])
@login_required
def get_fabric_combination():
    if request.method == 'GET':

        data_schema = FabricCombinationSchema(many=True)
        data = FabricCombination.query.all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/fabric_combination', methods=['POST'])
@login_required
def add_fabric_combination():
    if request.method == 'POST':
        payload = request.json
        print(payload)
        if len(payload['name']) != 0:

            check_data = FabricCombination.query.filter_by(name=payload['name'])
            if check_data.first():
...
@bp.route('/edit/fabric_combination', methods=['POST'])
@login_required
def edit_fabric_combination():
    if request.method == 'POST':
...
@bp.route('/delete/fabric_combination', methods=['POST'])
@login_required
def delete_fabric_combination():
    if request.method == 'POST':
        payload = request.json
