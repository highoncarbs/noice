...
from app.basic_master.model import Location, LocationSchema
...
def add_location():
    if request.method == 'POST':
        ...
        if len(payload['name']) != 0:
            ...
            check_data = Location.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
...
def edit_location():
    if request.method == 'POST':
        ...
        if payload['name'] is not None:
            ...
            check_data = Location.query.filter_by(
...
def delete_location():
    if request.method == 'POST':
        ...
        if check_data.first():
            if len(check_data.first().task_location_act) is int(0):
