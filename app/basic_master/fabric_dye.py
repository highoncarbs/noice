...
from app.basic_master.model import FabricDye, FabricDyeSchema
...
@bp.route('/get/fabric_dye', methods=['GET'])
@login_required
def get_fabric_dye():
    if request.method == 'GET':

        data_schema = FabricDyeSchema(many=True)
        data = FabricDye.query.all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/fabric_dye', methods=['POST'])
@login_required
def add_fabric_dye():
    if request.method == 'POST':
        ...
        if len(payload['name']) != 0:
            ...
            check_data = FabricDye.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
...
@bp.route('/edit/fabric_dye', methods=['POST'])
@login_required
def edit_fabric_dye():
    if request.method == 'POST':
        ...
        if payload['name'] is not None:
            ...
            check_data = FabricDye.query.filter_by(
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = FabricDye.query.filter_by(
                        id=payload['id']).first()
...
@bp.route('/delete/fabric_dye', methods=['POST'])
@login_required
def delete_fabric_dye():
    if request.method == 'POST':
        payload = request.json
        check_data = FabricDye.query.filter_by(id=payload['id'])
        if check_data.first():
            if len(check_data.first().fabric_dye_goods) is int(0):
                try:
                    check_data.delete()
                    db.session.commit()
