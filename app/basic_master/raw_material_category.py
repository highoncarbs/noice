...
from app.basic_master.model import RawMaterialCategory, RawMaterialCategorySchema
...
@bp.route('/get/raw_material_category', methods=['GET'])
@login_required
def get_raw_material_category():
    if request.method == 'GET':

        data_schema = RawMaterialCategorySchema(many=True)
        data = RawMaterialCategory.query.all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/raw_material_category', methods=['POST'])
@login_required
def add_raw_material_category():
    if request.method == 'POST':
        ...
        if len(payload['name']) != 0:
            ...
            check_data = RawMaterialCategory.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
                return jsonify({'message': 'Data - '+check_data.first().name+' already exists.'})
            else:
                try:
                    new_data = RawMaterialCategory(
...
@bp.route('/edit/raw_material_category', methods=['POST'])
@login_required
def edit_raw_material_category():
    if request.method == 'POST':
        
        payload = request.json
        if payload['name'] is not None:

            check_data = RawMaterialCategory.query.filter_by(
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = RawMaterialCategory.query.filter_by(
                        id=payload['id']).first()
...
@bp.route('/delete/raw_material_category', methods=['POST'])
@login_required
def delete_raw_material_category():
    if request.method == 'POST':
        payload = request.json
        check_data = RawMaterialCategory.query.filter_by(id=payload['id'])
        if check_data.first():
            if len(check_data.first().raw_material_category_goods) is int(0):
