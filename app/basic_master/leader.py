...
from app.basic_master.model import Leader, LeaderSchema
...
@bp.route('/get/leader', methods=['GET'])
@login_required
def get_leader():
    if request.method == 'GET':

        data_schema = LeaderSchema(many=True)
        data = Leader.query.all()
        print(data)
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@bp.route('/add/leader', methods=['POST'])
@login_required
def add_leader():
    if request.method == 'POST':
        ...
        if len(payload['name']) != 0:
            ...
            check_data = Leader.query.filter_by(name=payload['name'].lower().strip())
            if check_data.first():
...
@bp.route('/edit/leader', methods=['POST'])
@login_required
def edit_leader():
    if request.method == 'POST':
        ...
        if payload['name'] is not None:
            ...
            check_data = Leader.query.filter_by(
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = Leader.query.filter_by(
                        id=payload['id']).first()
...
@bp.route('/delete/leader', methods=['POST'])
@login_required
def delete_leader():
    if request.method == 'POST':
        payload = request.json
